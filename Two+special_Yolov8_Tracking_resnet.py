...
def Driver_Passenger(DPM, dir, track_id):
    ...
    if track_id == '7' and len(person) > 1:
        print(len(person))
...
def mask_img(frame, g):
    ...
    crop_img = frame[int(y1) : int(y1) + int(h), int(x1) : int(x1) + int(w)]
    blur_img = cv2.blur(crop_img, (30, 30))
    frame[y1 : y2, x1 : x2] = blur_img
...
def one_video_pipeline(video_id, video_name):
    ...
    video = cv2.VideoWriter(os.path.join('/home/Ricky/AI_City_Track5/Test_res_TSYTR_mask', 'Two+special_' + video_name), 
                            fourcc, 5, (1920, 1080))
    ...
    for t in tracker: # every frame
        frame, mask_frame = t.orig_img, t.orig_img.copy()
        track_id = ''
        for g in t.boxes.data: # every group
            ...
            for p in person:
                mask_frame = mask_img(mask_frame, p)

            for m in motor:
                mask_frame = mask_img(mask_frame, m)
        ...
        special_stage(video_id, frame_id, mask_frame, frame)
...
def special_stage(video_id, frame_id, mask_frame, frame):
    cls = ['motorbike', 'D', 'P1', 'P2']
    res = s_yolo.predict(source = mask_frame, conf = 0.15)
    # cls: m, D, P1, P2
    for r in res:
        img = r.orig_img
        for b in r.boxes.data:
            x1, y1, x2, y2 = int(b[0]), int(b[1]), int(b[2]), int(b[3])
            conf, cls_id = round(float(b[-2]), 2), int(b[-1])
            x1, y1 = out_of_range(x1, y1)
            x2, y2 = out_of_range(x2, y2)

            if cls_id == 0:
                motor = [[x1, y1, x2, y2, conf, 'SP', cls[cls_id]]]
                out_frame(frame, [], motor)
                output_res(video_id, frame_id, [], motor)
            else:
                person = [[x1, y1, x2, y2, conf, 'SP', cls[cls_id]]]
                get_helmet(img, person)
                out_frame(frame, person, [])
                output_res(video_id, frame_id, person, [])
...
if __name__ == '__main__':
    ...
    device_num = 1
    video_path = '/mnt/Nami/2023_AI_City_challenge_datasets/Track_5/aicity2023_track5_test/videos'
    res_file_name = 'res_TSYTR_mask_5_5_15.txt'
    ...
    s_yolo = YOLO('weights/FHD_Fullset_special_stage_best.pt') # detect special case(only motor or person)
    classifer = resnet_101(num_classes = 2).cuda('cuda:' + str(device_num))
    classifer.load_state_dict(torch.load('resnet101_DA.pt'))
    classifer.eval()
    

    all_result(arg.start, arg.end)
    #one_video_pipeline('25', '025.mp4')